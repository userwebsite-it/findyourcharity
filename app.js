// FindYourCharity — app logic
const SITE_EMAIL = "Ayan.bahulkar16@gmail.com";
const PAGE_SIZE = 24; // pagination size
const DATA_URL = "data/charities.json";

const sections = {
  cause: ["Health & Medical Care", "Education & Youth", "Environment & Conservation", "Disaster Relief & Emergency Aid", "Hunger, Poverty & Housing", "Animal Welfare", "Arts, Culture & Humanities", "Human Rights & Equality", "Community Development", "Religion & Faith-Based", "Science & Technology for Good", "Sports & Recreation"],
  size: ["Major Global Charities", "National Non-Profits", "Local Community Charities", "Small Grassroots Orgs", "Verified 501(c)(3) Charities"],
  special: ["Veterans & Military Support", "Children & Families in Need", "Senior & Elderly Support", "LGBTQ+ Organizations", "Disability Advocacy & Support", "Indigenous & Tribal Communities"]
};
...
function doSearchNavigate(term) {
  if (!term) return;
  // Navigate to a pseudo "browse all" in category template with search
...
function mountIndex() {
  ...
  form?.addEventListener('submit', (e) => {
    e.preventDefault();
    doSearchNavigate(input.value.trim());
  });
}
...
function mountCategoryPage() {
  ...
  const search = params.get('search') ? fromSlug(params.get('search')) : "";
  qs('#catTitle').textContent = category;
  if (search) qs('#catSearch').value = search;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.body.dataset.page === "find") mountFindPage();
  if (document.body.dataset.page === "category") mountCategoryPage();
});
