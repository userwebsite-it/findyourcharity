...
const DATA_URL="data/charities.json"; const PAGE_SIZE=24;
...
async function loadData(){try{const r=await fetch(DATA_URL,{cache:'no-store'});if(!r.ok)throw new Error(r.statusText);return await r.json();}catch(e){console.error('Load error',e);return [];}} 
...
function mountIndex(){
  ...
  f.addEventListener("submit",e=>{e.preventDefault();if(!i.value.trim())return;location.href=`category.html?category=${encodeURIComponent('Browse All')}&search=${encodeURIComponent(i.value.trim())}`;});
  loadData().then(all=>{
...
function mountCategory(){
  ...
  function apply(){
    ...
    const loc=qs("#loc").value.trim().toLowerCase();
    view = all.filter(c=>{
  ...
  function render(page){
    ...
    for(let p=1;p<=pages;p++){ const b=document.createElement("button"); b.className="btn"; b.textContent=p; if(p===page){b.style.background="var(--primary)"; b.style.color="#fff"} b.addEventListener("click",()=>render(p)); pag.appendChild(b); }
  }

  loadData().then(d=>{ all=d; apply(); });
